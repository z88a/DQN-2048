...
filename = r'dqn_2048_2022-11-27_20-20-42.pkl'
agent = DQN.DQN()
...
class MyFrame(wx.Frame):
    ...
    def on_key(self, event):
        ...
        if key_code == wx.WXK_UP:
            ...
            print('auto2048')
            while not (self.is_gameover() or self.is_win()):
                datain = one_hot(np.array(self.tile_values))
                action = agent.select_action(datain,False)
                print(action_space[action])
                self.move(action_space[action])
                self.draw_titles()
                time.sleep(0.1)
        else:
            print(key_code)
    ...
    def is_gameover(self):
        return not any(self.move_is_possible(move) for move in {'Left','Right','Up','Down'}) or self.ill_move>=10
